...
- [x] ~~**`VERSAND_TOKEN_SECRET` setzen.**~~ — gesetzt (32 Byte Zufall), Production. Ohne die Variable verweigern alle Versandrouten den Dienst; das ist Absicht.
- [x] ~~**Format der Adressvorschau bestätigen.**~~ — **bestätigt: PNG** (212 KB im Staging-Lauf). Die Route liefert den von eBrief gemeldeten Typ aus. Ursprünglicher Text: Die Spezifikation beschreibt `GET /Docs/{docId}/FileWithMark` als JSON-Umschlag mit base64-Inhalt und im Summary als **PNG** der ersten Seite, nicht als PDF. `/api/versand/adressvorschau` liefert deshalb den Typ aus, den eBrief meldet. Der Spike schreibt die Datei mit der Endung, die eBrief nennt — daran ist zu sehen, was wirklich kommt.
- [x] ~~Nach dem Spike prüfen, ob die Annahme „committeter Job ohne Distribution kostet nichts" hält~~ — **belegt am 31.07.2026.** `UNPROCESSED → COMITTED → USER_WAIT_FOR_SHOPPING`, dann Stillstand. Zusätzlich belegt: `POST /Jobs/distribution` wirkt direkt aus diesem Status, `POST /Jobs/payment` ist nicht nötig. Siehe `docs/ebrief/API-SCHEMA.md`.
...
- [ ] Prüfen, ob der am 31.07.2026 auf Staging distribuierte Job 2543114338 tatsächlich als Brief ankommt. Erwartet wird nichts — falls doch, druckt und versendet auch das Testsystem, und der Umgang damit muss neu bewertet werden.
