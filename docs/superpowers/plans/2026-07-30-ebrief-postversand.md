...
**Tech Stack:** Next.js 16 App Router, TypeScript, jsPDF (serverseitig, mit eingebetteter Liberation Sans), Stripe Node SDK, Playwright.
...
| `src/lib/fonts/liberationSans.ts` | Base64-TTF für die Einbettung |
...
- Create: `src/lib/fonts/liberationSans.ts` (generiert)

- [ ] **Step 1: Schrift beschaffen**

Liberation Sans ist unter SIL OFL lizenziert. Regular genügt — der Brieftext braucht keinen Fettschnitt.

```bash
mkdir -p tmp-font && curl -sL "https://github.com/liberationfonts/liberation-fonts/releases/download/2.1.5/liberation-fonts-ttf-2.1.5.tar.gz" | tar -xz -C tmp-font --strip-components=1 && ls tmp-font/LiberationSans-Regular.ttf
```

Expected: `tmp-font/LiberationSans-Regular.ttf` existiert.
...
```ts
/**
 * Wandelt eine TTF in ein TS-Modul mit Base64-String, damit jsPDF die Schrift
 * einbetten kann. Der Umweg über eine Datei auf der Platte scheitert auf
 * Vercel am File-Tracing; als Modul wird die Schrift zuverlässig mitgebündelt.
 *
 * Aufruf: npx tsx scripts/build-font.ts tmp-font/LiberationSans-Regular.ttf
 */
import { readFileSync, writeFileSync, mkdirSync } from "node:fs";

const quelle = process.argv[2];
if (!quelle) {
  console.error("Aufruf: npx tsx scripts/build-font.ts <pfad.ttf>");
  process.exit(1);
}
...
writeFileSync(
  "src/lib/fonts/liberationSans.ts",
  `// Generiert von scripts/build-font.ts — nicht von Hand bearbeiten.\n` +
    `// Liberation Sans Regular, SIL Open Font License 1.1.\n` +
    `// Metrisch kompatibel zu Arial, das die eBrief-Abstandsvorlage als\n` +
    `// geeignete Adressschrift nennt. eBrief verlangt vollständige Einbettung.\n` +
    `export const LIBERATION_SANS_REGULAR_BASE64 = "${base64}";\n`
);
console.log(`Geschrieben: src/lib/fonts/liberationSans.ts (${base64.length} Zeichen)`);
```

- [ ] **Step 3: Schriftmodul erzeugen und aufräumen**

```bash
npx tsx scripts/build-font.ts tmp-font/LiberationSans-Regular.ttf && rm -rf tmp-font
```

Expected: `src/lib/fonts/liberationSans.ts` existiert.
...
```bash
git add scripts/build-font.ts src/lib/fonts/liberationSans.ts
git commit -m "Embed Liberation Sans for the dispatch PDF"
```
...
import { LIBERATION_SANS_REGULAR_BASE64 } from "./fonts/liberationSans";
...
function registriereSchrift(doc: jsPDF): void {
  ...
  doc.addFont("LiberationSans-Regular.ttf", SCHRIFT, "normal");
