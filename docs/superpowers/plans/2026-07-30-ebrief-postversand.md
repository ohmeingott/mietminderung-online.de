...
- [ ] **Zeitstempelfeld des eBrief-Jobs verifizieren.** Die Doku zeigt nirgends ein Beispiel-Job-Objekt, deshalb rät der Cleanup-Cron das Feld (`CreatedAt`, `CreationDate`, `CreateDate`, `Created`). Trifft keines zu, löscht er nichts und protokolliert „Feld heißt vermutlich anders" — er scheitert also sicher, braucht dann aber eine Einzeiler-Korrektur. Der Spike beantwortet das nebenbei.
